                            ...
                            <form action="{{ route('member.service.update', [$service->id]) }}" method="POST">
                                ...
                                <div class="">
                                    <div class="px-4 py-5 sm:p-6">
                                        <div class="grid grid-cols-6 gap-6">
                                            ...
                                            <div class="col-span-6">
                                                ...
                                                <input placeholder="Jelaskan Service apa yang kamu tawarkan?" type="text" name="description" id="description" autocomplete="description"
                                                class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500
                                                focus:border-green-500 sm:text-sm" value="{{ $service->description ?? '' }}" required>
                                            ...
                                            <div class="col-span-6">
                                                ...
                                                @forelse ($advantage_service as $advantage_item)
                                                    <input placeholder="Keunggulan Service" type="text" name="{{ ('advantage-services['.$advantage_item->id.']') }}" id="advantage-services" autocomplete="advantage-services" class="block w-full py-3 mt-1
                                                    border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                            ...
                                            <div class="col-span-6 -mb-6">
                                                <label for="revision-limit & revision" class="block mb-3 font-medium text-gray-700 text-md">Estimasi Service & Jumlah Revisi</label>
                                            </div>

                                            <div class="col-span-6 sm:col-span-3">
                                                <select id="delivery_time" name="delivery_time" autocomplete="delivery_time" class="block w-full px-3 py-3 pr-10 mt-1
                                                bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500
                                                focus:border-indigo-500 sm:text-sm" required>
                                                    <option>Butuh Berapa hari service kamu selesai?</option>
                                                    <option value="2" {{ $service->delivery_time == '2' ? 'selected' : ''  }} >2 Days</option>
                                                    <option value="4" {{ $service->delivery_time == '4' ? 'selected' : ''  }} >4 Days</option>
                                                    <option value="8" {{ $service->delivery_time == '8' ? 'selected' : ''  }} >8 Days</option>
                                                    <option value="16" {{ $service->delivery_time == '16' ? 'selected' : ''  }} >16 Days</option>
                                                    <option value="32" {{ $service->delivery_time == '32' ? 'selected' : ''  }} >32 Days</option>
                                                </select>
                                            </div>

                                            <div class="col-span-6 sm:col-span-3">
                                                <select id="revision_limit" name="revision_limit" autocomplete="revision_limit" class="block w-full px-3 py-3 pr-10 mt-1
                                                 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500
                                                 focus:border-indigo-500 sm:text-sm" required>
                                                    <option>Maksimal Revisi service kamu </option>
                                                    <option value="2" {{ $service->revision_limit == '2' ? 'selected' : ''  }} >2 Revision</option>
                                                    <option value="5" {{ $service->revision_limit == '5' ? 'selected' : ''  }} >5 Revision</option>
                                                    <option value="7" {{ $service->revision_limit == '7' ? 'selected' : ''  }} >7 Revision</option>
                                                    <option value="10" {{ $service->revision_limit == '10' ? 'selected' : ''  }} >10 Revision</option>
                                                    <option value="12" {{ $service->revision_limit == '12' ? 'selected' : ''  }} >12 Revision</option>
                                                </select>
                                            </div>

                                            <div class="col-span-6">
                                                <label for="price" class="block mb-3 font-medium text-gray-700 text-md">Harga Service Kamu</label>
                                                <input placeholder="Total Harga Service Kamu" type="number" name="price" id="price" autocomplete="price" class="block w-full py-3
                                                mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                            ...
                                            <div class="col-span-6">
                                                ...
                                                @forelse ($advantage_user as $advantage_user_item)
                                                    <input placeholder="Keunggulan Kamu" type="text" name="{{ ('advantage-users['.$advantage_user_item->id.']') }}" id="advantage-users"
                                                    autocomplete="advantage-users" class="block w-full py-3 mt-1
                                                    border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                            ...
                                            <div class="col-span-6">
                                                ...
                                                class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500
                                                focus:border-green-500 sm:text-sm" value="{{ $service->note ?? '' }}" required>
                                            ...
                                            <div class="col-span-6">
                                                ...
                                                @forelse ($tagline as $tagline_item)
                                                    <input placeholder="Tagline" type="text" name="{{ ('taglines['.$tagline_item->id.']') }}"
                                                    id="taglines"
                                                    autocomplete="taglines" class="block w-full py-3 mt-1
                                                    border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                    ...
                                    <div class="px-4 py-3 text-right sm:px-6">
                                        ...
                                        <a href="{{ route('member.service.index') }}" type="button" class="inline-flex justify-center px-4 py-2 mr-4 text-sm
                                        font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm
                                        hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300" onclick="return confirm('Are you sure want to cancel?, Any changes you make will not be saved')">
                                            Cancel
                                        </a>

                                        <button type="submit" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm
                                        hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return confirm('Are you sure want to submit this data ?')">
                                            Save Changes
                                        </button>
