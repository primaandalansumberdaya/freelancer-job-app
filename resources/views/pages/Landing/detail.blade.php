                    ...
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        ...
                        <div class="flex mt-2 flex-nowrap">
                            @forelse ($thumbnail as $item)
                                ...
                                @onclick="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})"
                                src="url(Storage::url(( $item->thumbnail ))" alt="thumbnail service" class="inline-block mr-2 rounded-lg
                                cursor-pointer h-20 w-36 object-cover">
                            @empty
                                {{-- empty --}}
                            @endforelse
                        </div>
                    ...
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                            ...
                            <ul class="mb-8 list-check">
                                @forelse ($experience as $experience_item)
                                     <li class="pl-10 my-4">{{ $experience_item->experience }}</span>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                            </ul>
                            ...
                            <div class="mb-8 skills">
                                @forelse ($tagline as $tagline_item)
                                    <span class="inline-block px-3 py-1 mr-2 rounded bg-serv-services-bg">{{ $tagline_item->tagline }}</span>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                            </div>
                    ...
                    <div class="px-4 pt-4 pb-2 features-list">
                        <ul class="mb-4 text-sm list-check">
                            @forelse ($advantage_user as $advantage_user_item)
                                    <li class="pl-10 my-4">{{ $advantage_user_item->advantage ?? '' }}</li>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                        </ul>
